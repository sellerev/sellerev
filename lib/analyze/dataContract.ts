...
import { AsinSnapshot } from "@/lib/amazon/asinData";
...
// ============================================================================
// ASIN ANALYZE CONTRACT
// ============================================================================

export interface AsinAnalyzeResponse {
  // Top-Level Metadata
  asin: string;
  marketplace: Marketplace;
  ownership: "own" | "competitor";
  data_sources: {
    listing: "sp_api" | "manual" | "rainforest";
    competitors: "rainforest";
  };
  confidence: Confidence;

  // A) ASIN Snapshot (Top Bar)
  asin_snapshot: {
    price: number;
    rating: number;
    review_count: number;
  ...
  listing_quality: {
    ...
    images: {
      ...
      infographic_present: boolean;
    };
  };

  // C) Page-1 Benchmarks (Comparative)
  page1_benchmarks: {
    avg_price: number;
    avg_rating: number;
  ...
  ai_context: {
    mode: "asin";
    asin_snapshot: AsinAnalyzeResponse["asin_snapshot"];
    listing_quality: AsinAnalyzeResponse["listing_quality"];
    page1_benchmarks: AsinAnalyzeResponse["page1_benchmarks"];
    margin_snapshot: AsinAnalyzeResponse["margin_snapshot"];
    signals: AsinAnalyzeResponse["signals"];
  };
}
...
export function buildAsinAnalyzeResponse(
  asin: string,
  asinData: AsinSnapshot,
  marginSnapshot: MarginSnapshot,
  marketplace: Marketplace = "US",
  ownership: "own" | "competitor" = "competitor"
): AsinAnalyzeResponse {
  // Build ASIN snapshot
  const asinSnapshot = {
    price: asinData.price || 0,
    rating: asinData.rating || 0,
    review_count: asinData.reviews || 0,
    bsr: asinData.bsr || null,
    estimated_monthly_units: 0, // TODO: Calculate from BSR/position if available
    estimated_monthly_revenue: 0, // TODO: Calculate from BSR/position if available
    category: "", // TODO: Extract from ASIN data if available
    fulfillment: normalizeFulfillment(asinData.fulfillment),
  };
  
  // Build listing quality (placeholder - needs actual listing data)
  const listingQuality = {
    ...
    images: {
      image_count: 0,
      lifestyle_present: false,
      infographic_present: false,
    },
  };
  
  // Build page1 benchmarks (placeholder - needs competitor data)
  const page1Benchmarks = {
    avg_price: 0,
    avg_rating: 0,
    avg_review_count: 0,
    avg_title_length: 0,
  ...
  const marginSnapshotContract = {
    selling_price: marginSnapshot.assumed_price || 0,
    cogs: marginSnapshot.estimated_cogs_min || null, // Use min as single value
    fba_fees: marginSnapshot.estimated_fba_fee || 0,
    net_margin_pct: marginSnapshot.net_margin_min_pct || null,
    breakeven_price: marginSnapshot.breakeven_price_min || null,
  };
  
  // Build signals (placeholder - needs actual calculation)
  const signals = {
    pricing_position: "at" as PricingPosition,
    review_position: "at" as ReviewPosition,
    listing_strength: "average" as ListingStrength,
  };
  
  // Build AI context (read-only copy)
  const aiContext = {
    mode: "asin" as const,
    asin_snapshot: asinSnapshot,
    listing_quality: listingQuality,
    page1_benchmarks: page1Benchmarks,
    margin_snapshot: marginSnapshotContract,
    signals,
  };
