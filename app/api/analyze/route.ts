  ...
  try {
    ...
    if (keywordMarketData) {
      ...
      if (body.input_type === "keyword") {
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        // SAFETY MERGE: Ensure SP-API BSR is merged into rawListings before canonical builder
        // ═══════════════════════════════════════════════════════════════════════════
        // CRITICAL: SP-API Catalog extracts BSR, but it must be in rawListings before buildKeywordPageOne()
        // This ensures BSR is available even if merge in fetchKeywordMarketSnapshot didn't work correctly
        if (body.input_type === "keyword" && rawListings.length > 0 && supabase) {
          const page1Asins = Array.from(new Set(
            rawListings
              .slice(0, 20) // Check first 20 ASINs
              .map((l: any) => l.asin)
              .filter((asin: string | null) => asin && /^[A-Z0-9]{10}$/i.test(asin.trim().toUpperCase()))
              .map((asin: string) => asin.trim().toUpperCase())
          ));
          
          // Check for listings missing BSR but should have it (have bsr_source but no bsr value)
          const listingsMissingBSR = rawListings.filter((l: any) => {
            const hasBsrSource = (l as any).bsr_source === 'sp_api' || (l as any).bsr_source === 'sp_api_catalog';
            const missingBsr = (l.main_category_bsr === null || l.main_category_bsr === undefined) && 
                              (l.bsr === null || l.bsr === undefined);
            return hasBsrSource && missingBsr && l.asin;
          });
          
          if (listingsMissingBSR.length > 0) {
            console.log("⚠️ SAFETY_MERGE_NEEDED", {
              keyword: body.input_value,
              listings_missing_bsr: listingsMissingBSR.length,
              asins: listingsMissingBSR.map((l: any) => l.asin),
              message: "Listings have bsr_source but missing BSR value - attempting safety merge from DB",
            });
            
            // Query DB for BSR data
            try {
              const { data: bsrData } = await supabase
                .from('asin_core')
                .select('asin, main_category_bsr')
                .in('asin', listingsMissingBSR.map((l: any) => l.asin.toUpperCase()))
                .not('main_category_bsr', 'is', null);
              
              if (bsrData && bsrData.length > 0) {
                const bsrMap = new Map(bsrData.map((row: any) => [row.asin.toUpperCase(), row.main_category_bsr]));
                
                // Merge BSR into rawListings
                for (const listing of listingsMissingBSR) {
                  const asin = listing.asin?.toUpperCase();
                  if (asin && bsrMap.has(asin)) {
                    const bsr = bsrMap.get(asin);
                    if (bsr && bsr > 0) {
                      listing.main_category_bsr = bsr;
                      listing.bsr = bsr;
                      (listing as any).bsr_source = (listing as any).bsr_source || 'sp_api_catalog';
              ...
              console.error("❌ SAFETY_MERGE_DB_ERROR", {
                keyword: body.input_value,
                error: dbError instanceof Error ? dbError.message : String(dbError),
                message: "Failed to query DB for BSR data - continuing without safety merge",
              });
            }
          }
        }
